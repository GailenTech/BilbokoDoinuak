...
export function LoginPage() {
  ...
  const t = {
    ...
    successEmailSent: language === 'es'
      ...
      : 'Lotura bidali dugu zure emailera. Begiratu zure sarrerako ontzia.',
    successRegistration: language === 'es'
  ...
  const handleEmailRegister = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setSuccessMessage(t.successRegistration);
    }
    setIsLoading(false);
  };
  ...
  const handleMagicLink = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setSuccessMessage(t.successEmailSent);
    }
    setIsLoading(false);
  };

  return (
    <main className="min-h-[calc(100vh-64px)] flex items-center justify-center bg-gradient-to-b from-amber-50 to-white p-4">
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
          {/* Logos */}
          <div className="flex justify-center gap-4 mb-6">
            <img src={bilbaoLogo} alt="Ayuntamiento de Bilbao" className="h-16" />
            <img src={doinuakLogo} alt="Bilboko Doinuak" className="h-16" />
          </div>
          ...
          {mode === 'login' && (
            <form onSubmit={handleEmailLogin} className="space-y-4">
              ...
              >
                {isLoading ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : t.loginButton}
              </button>
            </form>
          )}
          ...
          {mode === 'register' && (
            <form onSubmit={handleEmailRegister} className="space-y-4">
              ...
              <button
                ...
                className="w-full py-3 bg-red-600 hover:bg-red-700 text-white font-medium rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isLoading ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : t.registerButton}
              </button>
            </form>
          )}
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white border-2 border-gray-200 rounded-xl text-base font-medium text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? (
              <Loader2 className="w-5 h-5 animate-spin" />
            ) : (
